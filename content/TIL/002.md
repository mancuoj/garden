...
> CSS is comprised of many different layout algorithms, known officially as “layout modes”. Each Layout algorithm is designed to solve a specific problem.


Default layout is _Flow layout_, in the way that inline elements display in the inline direction, fromt left to the right(according to the [Writing Modes](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_writing_modes)), and block elements display one after one, start at the top and move down the page.

When we flip `display` to `flex`, we create a flex formatting context, all children will be positioned according to the Flexbox layout algorithm.

Flexbox is all about arranging a group of items in a row or column, and giving us a _ridiculous_ amount of control over the distribution and alignment of those items.

As the name suggest, Flexbox is also all about flexibility, 
